...
client.on(Events.InteractionCreate, async (interaction) => {
    if (interaction.isChatInputCommand()) { // Lida com comandos de barra
        ...
        if (commandName === 'comprar') {
            ...
            // CORREÇÃO: Defer a resposta para evitar timeout de 3 segundos.
            await interaction.deferReply({ ephemeral: true });

            const productId = interaction.options.getString('id');
            const product = await getProductById(productId);
            if (!product) {
                return interaction.editReply({ content: `❌ Nenhum produto encontrado com o ID \`${productId}\`.` });
            }
            ...
            await interaction.editReply({
                content: `Você tem certeza que deseja deletar o item **${product.name}** (ID: \`${productId}\`)? Esta ação não pode ser desfeita.`,
                components: [row],
            });
